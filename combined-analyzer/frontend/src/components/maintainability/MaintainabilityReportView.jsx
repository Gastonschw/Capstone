...
import { getMaintainabilityAnalysis, pollAnalysis } from '../../api';
...
const styles = {
  ...
  priorityRecommendations: { backgroundColor: '#fff3e0', borderRadius: '12px', padding: '24px' },
  priorityTitle: { fontSize: '16px', fontWeight: '600', color: '#e65100', marginBottom: '16px' },
  priorityItem: { display: 'flex', alignItems: 'flex-start', gap: '12px', marginBottom: '12px' },
  priorityNumber: {
    width: '26px', height: '26px', borderRadius: '50%', backgroundColor: '#e65100',
...
export default function MaintainabilityReportView({ analysisId, onBack, chatModel, chatApiKey }) {
  ...
  const fetchAnalysis = async () => {
    setLoading(true);
    try {
      const data = await getMaintainabilityAnalysis(analysisId);
      setAnalysis(data);
      if (data.status === 'pending' || data.status === 'processing') {
        pollAnalysis('maintainability', analysisId, (updated) => {
          setAnalysis(updated);
    ...
    return (
      <div style={styles.container}>
        ...
        <div style={styles.loading}>
          ...
          <h3>Analyzing Maintainability Characteristics...</h3>
          <p style={{ marginTop: '10px', color: '#888' }}>
            Evaluating modularity, reusability, testability, and more
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div style={styles.container}>
        <div style={styles.header}>
          <h2 style={styles.title}>Maintainability Analysis Failed</h2>
  ...
  const characteristics = [
    { key: 'modularity', data: analysis.modularity_report, icon: '&#128230;' },
    { key: 'reusability', data: analysis.reusability_report, icon: '&#128260;' },
    { key: 'analysability', data: analysis.analysability_report, icon: '&#128269;' },
    { key: 'modifiability', data: analysis.modifiability_report, icon: '&#9999;' },
    { key: 'testability', data: analysis.testability_report, icon: '&#128295;' },
  ];
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h2 style={styles.title}>Maintainability Analysis Report</h2>
      ...
      <AnalysisChat analysisId={analysisId} analysisType="maintainability" chatModel={chatModel} chatApiKey={chatApiKey} />
    </div>
  );
}
