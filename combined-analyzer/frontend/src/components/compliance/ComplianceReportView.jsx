...
import { getComplianceAnalysis, pollAnalysis } from '../../api';
...
const styles = {
  ...
  priorityRecommendations: { backgroundColor: '#e3f2fd', borderRadius: '12px', padding: '24px' },
  priorityTitle: { fontSize: '16px', fontWeight: '600', color: '#1565c0', marginBottom: '16px' },
  priorityItem: { display: 'flex', alignItems: 'flex-start', gap: '12px', marginBottom: '12px' },
  priorityNumber: {
    width: '26px', height: '26px', borderRadius: '50%', backgroundColor: '#1565c0',
...
export default function ComplianceReportView({ analysisId, onBack, chatModel, chatApiKey }) {
  ...
  const fetchAnalysis = async () => {
    setLoading(true);
    try {
      const data = await getComplianceAnalysis(analysisId);
      setAnalysis(data);
      if (data.status === 'pending' || data.status === 'processing') {
        pollAnalysis('compliance', analysisId, (updated) => {
          setAnalysis(updated);
    ...
    return (
      <div style={styles.container}>
        ...
        <div style={styles.loading}>
          ...
          <h3>Analyzing Compliance Characteristics...</h3>
    ...
    return (
      <div style={styles.container}>
        <div style={styles.header}>
          <h2 style={styles.title}>Compliance Analysis Failed</h2>
  ...
  const characteristics = [
    { key: 'functional_completeness', data: analysis.functional_completeness_report, icon: '&#9989;' },
    { key: 'functional_correctness', data: analysis.functional_correctness_report, icon: '&#10004;' },
    { key: 'functional_appropriateness', data: analysis.functional_appropriateness_report, icon: '&#127919;' },
  ];
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h2 style={styles.title}>Compliance Analysis Report</h2>
      ...
      <AnalysisChat analysisId={analysisId} analysisType="compliance" chatModel={chatModel} chatApiKey={chatApiKey} />
    </div>
  );
}
