...
import { getCorrectnessAnalysis, pollAnalysis } from '../../api';
...
const styles = {
  ...
  priorityRecommendations: { backgroundColor: '#e8f5e9', borderRadius: '12px', padding: '24px' },
  priorityTitle: { fontSize: '16px', fontWeight: '600', color: '#2e7d32', marginBottom: '16px' },
  priorityItem: { display: 'flex', alignItems: 'flex-start', gap: '12px', marginBottom: '12px' },
  priorityNumber: {
    width: '26px', height: '26px', borderRadius: '50%', backgroundColor: '#2e7d32',
...
export default function CorrectnessReportView({ analysisId, onBack, chatModel, chatApiKey }) {
  ...
  const fetchAnalysis = async () => {
    setLoading(true);
    try {
      const data = await getCorrectnessAnalysis(analysisId);
      setAnalysis(data);
      if (data.status === 'pending' || data.status === 'processing') {
        pollAnalysis('correctness', analysisId, (updated) => {
          setAnalysis(updated);
    ...
    return (
      <div style={styles.container}>
        ...
        <div style={styles.loading}>
          ...
          <h3>Analyzing Correctness Characteristics...</h3>
          <p style={{ marginTop: '10px', color: '#888' }}>
            Evaluating computation accuracy, completeness, and appropriateness
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div style={styles.container}>
        <div style={styles.header}>
          <h2 style={styles.title}>Correctness Analysis Failed</h2>
  ...
  const characteristics = [
    { key: 'completeness', data: analysis.functional_completeness_correctness_report, icon: '&#128203;' },
    { key: 'accuracy', data: analysis.functional_correctness_accuracy_report, icon: '&#127919;' },
    { key: 'appropriateness', data: analysis.functional_appropriateness_correctness_report, icon: '&#9881;' },
  ];
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h2 style={styles.title}>Correctness Analysis Report</h2>
      ...
      <AnalysisChat analysisId={analysisId} analysisType="correctness" chatModel={chatModel} chatApiKey={chatApiKey} />
    </div>
  );
}
