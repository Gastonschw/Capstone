...
export default function IntegrityReportView({ analysisId, onBack, chatModel, chatApiKey }) {
  ...
  const fetchAnalysis = async () => {
    setLoading(true);
    try {
      const data = await getIntegrityAnalysis(analysisId);
      setAnalysis(data);

      if (data.status === 'pending' || data.status === 'processing') {
        pollAnalysis('integrity', analysisId, (updated) => {
          setAnalysis(updated);
      ...
      <div style={styles.container}>
        ...
        <div style={styles.loading}>
          ...
          <p style={{ marginTop: '10px', color: '#888' }}>
            Evaluating confidentiality, integrity, authenticity, and more
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div style={styles.container}>
        <div style={styles.header}>
          <h2 style={styles.title}>Integrity Analysis Failed</h2>
  ...
  const characteristics = [
    ...
    { key: 'resistance', data: analysis.resistance_report, icon: '&#128737;' },
  ];
