...
async def list_maintainability_analyses(
    repository_id: int,
    request: Request,
    db: AsyncSession = Depends(get_db),
):
...
async def get_maintainability_analysis(
    analysis_id: int,
    request: Request,
    db: AsyncSession = Depends(get_db),
):
    analysis = await require_analysis_access(request, db, analysis_id, MaintainabilityAnalysis)
